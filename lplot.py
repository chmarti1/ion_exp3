...
def zoom_ax(ax, pos, xlim=None, ylim=None, box=True, boxstyle={'fill':False,'color':'k'}):
    """ZOOM_AX
    zax = zoom_ax(ax, pos, xlim=None, ylim=None)
    
Given an axes, ax, zoom_ax() embeds new axes in the same figure that show an 
expanded view of the same data
"""

    default_box = {'fill':False, 'color':'k'}
    default_box.update(boxstyle)

    f = ax.get_figure()
    zax = f.add_axes(pos,label='ZAX')
    if xlim is None:
        xlim = ax.get_xlim()
    if ylim is None:
        ylim = ax.get_ylim()
    for this in ax.lines:
        fmt = { 'color':this.get_color(), 
                'dash_capstyle':this.get_dash_capstyle(),
                'dash_joinstyle':this.get_dash_joinstyle(),
                'fillstyle':this.get_fillstyle(),
                'linestyle':this.get_linestyle(),
                'linewidth':this.get_linewidth(),
                'marker':this.get_marker(),
                'markeredgecolor':this.get_markeredgecolor(),
                'markeredgewidth':this.get_markeredgewidth(),
                'markerfacecolor':this.get_markerfacecolor(),
                'markersize':this.get_markersize(),
                'markevery':this.get_markevery(),
                'solid_capstyle':this.get_solid_capstyle(),
                'solid_joinstyle':this.get_solid_joinstyle()
               }
        L = mpl.lines.Line2D(this.get_xdata(), this.get_ydata(), **fmt)
        zax.add_line(L)
    zax.set_xlim(xlim)
    zax.set_ylim(ylim)
    if box:
        ...
        Xlim = ax.get_xlim()
        Ylim = ax.get_ylim()
        bx0 = (xlim[0] - Xlim[0])/(Xlim[1]-Xlim[0])
        by0 = (ylim[0] - Ylim[0])/(Ylim[1]-Ylim[0])
        bx1 = (xlim[1] - Xlim[0])/(Xlim[1]-Xlim[0])
        by1 = (ylim[1] - Ylim[0])/(Ylim[1]-Ylim[0])
        B = mpl.patches.Rectangle([xlim[0], ylim[0]], xlim[1]-xlim[0], ylim[1]-ylim[0],
                                  **default_box)
        ax.add_patch(B)
